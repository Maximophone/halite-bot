...
random.seed(0)
logging.basicConfig(filename='routingbot.log',level=logging.CRITICAL)
...
def djikstra(start,radius,gamemap,cost,reward):
    vertex_set = set()
    dist = {}
    rew = {}
    prev = {}
    directions = {}
    for square in gamemap.neighbors(start,n=radius,include_self=True):
        dist[square] = 1e9
        prev[square] = None
        vertex_set.add(square)
    dist[start] = 0
    rew[start] = 0
    while len(vertex_set) > 0:
        square = min(vertex_set,key=lambda sq:dist[sq])
        vertex_set.remove(square)
        for d,neighbor in enumerate(gamemap.neighbors(square)):
            if neighbor not in vertex_set:
                continue
            alt = dist[square] + cost(neighbor)
            if alt < dist[neighbor]:
                dist[neighbor] = alt
                prev[neighbor] = square
                rew[neighbor] = reward(neighbor)
                directions[neighbor] = d
    return dist, rew, prev, directions

def find_best_path(start,radius,gamemap, cost, reward):
    dist, rew, prev, directions = djikstra(start,radius,gamemap,cost,reward)
    target = max(dist.keys(), key=lambda sq:rew[sq]/(dist[sq]+1))
    directions_dict = {sq:None for sq in gamemap}
    square = target
    while square != start:
        directions_dict[prev[square]] = directions[square]
        square = prev[square]
...
if __name__ == "__main__":
    ...
    directions_dict = find_best_path(start,int(min_distance_to_others/2),gamemap,cost,reward)
    logging.debug("Found Path")

    send_init("DjikstraBot")
