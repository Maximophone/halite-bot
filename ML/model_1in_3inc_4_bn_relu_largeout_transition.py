...
def main(old_model_file,new_model_file):
    ...
    special_weights = weights[22][0]
    special_biases = weights[22][1]
    special_weights[:,:,:,32:] = np.random.normal(0,0.1,size=(3,3,32,32))
    # for i in range(32):
    #     special_weights[1,1,i,i] = 1.

    cv4 = Convolution2D(64,3,3,border_mode='same',weights=(special_weights,special_biases))(act3)
    # bn4 = BatchNormalization()(cv4)
    act4 = Activation("relu")(cv4)

    special_weights = weights[24][0]
    special_biases = weights[24][1]
    special_weights[:,:,32:,:] = np.random.normal(0,0.1,size=(3,3,32,128))

    # special_weights = np.zeros((3,3,64,128))
    # special_weights[:,:,:32,:] = weights[22][0]
    # special_biases = weights[22][1]

    cv5 = Convolution2D(128,3,3,border_mode='same',weights=(special_weights,special_biases))(act4)
    bn5 = BatchNormalization(weights=weights[25])(cv5)
    act5 = Activation("relu")(bn5)

    output = Convolution2D(5,3,3,activation='relu',border_mode='same',weights=weights[27])(act5)

    # cv6 = Convolution2D(8,3,3,activation='relu',border_mode='same')(act5)
    # bn6 = BatchNormalization()(cv6)
    # act6 = Activation("relu")(bn6)

    # output = Convolution2D(5,20,20,activation='relu',border_mode='same')(act6)
